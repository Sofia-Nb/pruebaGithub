...
function letraAbcdario ($palabra){
    ...
    for ($i = 0; $i < strlen($palabra); $i++) { // La función "strlen()" devuelve la longitud de un texto
        ...
        if ((ord($caracter) >= 65 && ord($caracter) <= 77) && !($esVocal)) { // La función "ord()" devuelve el código ASCII de un carácter
            $contadorHastaM = $contadorHastaM + 2;
        }
        // Comprueba que el caracter sea consonantes mayor a M y que NO SEA VOCAL
        if ((ord($caracter) > 77 && ord($caracter) <= 90) && !($esVocal)) { 
            $contadorDesdeM = $contadorHastaM + 3;
        }
    }
 ...
 * @return int 
 */
function obtenerPuntajeWordix($intentos, $vocalCons){
    // 
    if ($intentos != 0) {
        $puntajeTotal = $intentos + $vocalCons;
    }else {
        $puntajeTotal = 0; 
    }
    ...
    */
    ...
    do {
        ...
        $palabraIntento = palabra5Letras();
         for ($i=0; $i<$j; $i++){
            if ($palabraAcum[$i] != $palabraIntento){
              $palabraAcum[$i]=$palabraIntento;
            }else{
                echo "La palabra ya fue usada recientemente, vuelva a intentar. \n";
                $j++;
                $palabraIntento = palabra5Letras();
                $i=-1;
            }
        ...
        $nroIntento++;
        $puntajeVocalConsonante = letraAbcdario($palabraIntento);
        $puntajeVocalConsSuma = $puntajeVocalConsSuma + $puntajeVocalConsonante;

    } while ($nroIntento <= CANT_INTENTOS && !$ganoElIntento);

    if ($ganoElIntento) {
        $nroIntento--;
        $puntajeDeIntentos=calcularPuntajeIntentos($nroIntento);
        $puntaje = obtenerPuntajeWordix($puntajeDeIntentos, $puntajeVocalConsSuma);
